        ...
        if(!isset($available_amount)){
            ...
            if($available_amount >= $total_amount){
                ...
                foreach($bid_lists as $bid_list){
                    ...
                    $sql = "INSERT INTO betting_history ( user_id, market_id, game_name, number, points, game_type, created_at, updated_at) VALUES
                    (:user_id, :market_id, :game_name, :number, :points, :game_type, :created_at, :updated_at)";
                    $query = $con->prepare($sql);
                    $query->bindParam(':user_id', $user_id, PDO::PARAM_STR);
